...
for(let i = 0; i < numbersToDraw.length; i++){
    const numberGlyph = new (0, _glyph.Glyph)(ctx, canvas.clientHeight, 200, {
        x: 200 * i,
        y: 0
    }, {
...
class Glyph {
    ...
    set number(value) {
        ...
        this.digitsArray = (0, _helpersDefault.default).splitNumberIntoDigits(this.number);
    ...
    draw() {
        ...
        if (this.consecutiveNumbers.length !== 1 || this.consecutiveNumbers[0].occurence !== this.options.maxDigits - 1) {
            // Draw small circle
            const circleCoord = this.getStartCircleCoordinates(this.digitsCoordinates[0], this.digitsCoordinates[1]);
            this.ctx.beginPath();
        ...
        this.digitsArray.forEach((digit, index)=>{
            if (index !== 0 && (0, _helpersDefault.default).isSameDirection(this.digitsDirections[index], this.digitsDirections[index - 1]) && digit !== this.digitsArray[index - 1]) {
                ...
                const angle = Math.atan2(nextCoordinates.x - coordinates.x, nextCoordinates.y - coordinates.y);
                console.log(angle, angle * 180 / Math.PI);
                const perpendicularAngle = angle + Math.PI / 2;
                console.log(perpendicularAngle, perpendicularAngle * 180 / Math.PI);
                debugger;
                const direction = Math.cos(perpendicularAngle);
                const perpendicularDirection = (0, _helpersDefault.default).getPerpendicularDirection(this.digitsDirections[index]);
                this.ctx.beginPath();
                this.ctx.lineCap = "butt";
                this.ctx.lineJoin = "miter";
                this.ctx.moveTo(coordinates.x + direction * (this.style.sameDirectionLineLength / -2), coordinates.y + direction * (this.style.sameDirectionLineLength / -2));
                this.ctx.lineTo(coordinates.x + direction * (this.style.sameDirectionLineLength / 2), coordinates.y + direction * (this.style.sameDirectionLineLength / 2));
                this.ctx.stroke();
                this.ctx.closePath();
                this.ctx.lineCap = "round";
                this.ctx.lineJoin = "round";
        ...
        this.digitsDirections.forEach((direction, index)=>{
            ...
            if ((0, _helpersDefault.default).isOppositiveDirection(direction, this.digitsDirections[index - 1])) {
    ...
    getStartCircleCoordinates(firstCoord, secondCoord) {
        return {
...
//# sourceMappingURL=number-glyph.0f77c784.js.map
